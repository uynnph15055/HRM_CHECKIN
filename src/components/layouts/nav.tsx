import { SideLink } from '@/config/side-links';
import { cn } from '@/lib/utils';
import { Link } from '@tanstack/react-router';
import { AiOutlineDown } from 'react-icons/ai';

import { Button, buttonVariants } from '../customs/button';
...
// TODO: Fix use tanstack router
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const useCheckActiveNav = (str: string) => {
  return false;
};

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const checkActiveNav = (str: string) => {
  return false;
};
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        'h-12 justify-start text-wrap rounded-none px-6 font-normal',
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          'group h-12 w-full justify-start rounded-none px-6 font-normal',
        )}
      >
        <div className='mr-2'>{icon}</div>
        {title}
